...
chmod 400 "$5".pem
ssh -i "$5".pem -o StrictHostKeyChecking=no ubuntu@$IP 'sudo apt-get update; sudo apt-get install python awscli jq --assume-yes; printf "'${6}'\n'${7}'\n'${8}'\n\n" | /usr/bin/aws configure;/usr/bin/aws s3 cp s3://testrunkostic/ ~/.ssh/ --recursive;sudo chmod 400 ~/.ssh/id_rsa;/usr/bin/aws s3 cp s3://'$5'/tempkey/'$5'.pem .;sudo chmod 400 '$5'.pem; git clone git@github.com:kosticlab/diabimmune.git;cd diabimmune;git checkout jacob;cd bin;sudo chmod +x *.sh;cd ..;./bin/primary_node_init.sh '$1' '$2' '$3' '$4' '$5' '$6' '$7' '$9' '${10}' 2> error.log &'
